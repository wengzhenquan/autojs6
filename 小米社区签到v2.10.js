...
function 解锁() {
    ...
    if (!isNaN(count) && jpso.size() > 0 && count > 0) {
    ...
    if (v53) {
        ...
        if (config.坐标点击) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                toastLog("点击" + config.x + "," + config.y, "forcible")
                sleep(400);
            }

        } else {
            toastLog("桌面所有屏幕，全力搜索小程序图标启动！", "forcible")

            let xcx = desc("小米社区").clickable().findOne(3666);
        ...
        waitForPackage("com.tencent.mm", 10, 500);
    }
...
function permissionv() {
    ...
    if ("Xiaomi".equals(brand) || "Xiaomi".equals(manufacturer)) {
        ...
        console.log("当前设备不是小米设备，不执行小米手机的权限检查逻辑。");
    }
...
function main() {
    ...
    sleep(300);
    home();
    notice(String('全部操作已完成(' + nowDate().substr(5, 14) + ')'), String(text));
