...
function posts(n) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        //if(i/2===0) scrollUp() else scrollDown();
...
function levelResult() {
    ...
    let outP2 = (`今日获得：${成长值记录.今日获得()}`).padEnd(12) + (`距离升级还需：${成长值记录.距离升级还需()}`);
    log(outP1);
    log(outP2);
    log("----------------------------");
    log("详细记录：");
