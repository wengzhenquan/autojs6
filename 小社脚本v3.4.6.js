...
function skipAd() {
    //开屏广告
    let skilCloseBtn = textStartsWith("跳过").findOne(688);
    if (skilCloseBtn) {
        skilCloseBtn.click();
        log("跳过了开屏广告!");
    }
...
function findCenter() {
    ...
    // 等待验证图片的加载
    sleep(2000);
    let result = wait(() => existsOne(text("关闭验证"), text("刷新验证"), text("帮助反馈"), text("视觉障碍")), 3, 1000);
...
let urls = [
    ...
    "http://xcjd.wzqw.zone.id/upload", //9


    "http://up.kuandana.v6.rocks/upload", //9
    "http://up.风中拾叶.top/upload", //10
