 ...
 function launchAPP(packageName) {
     ...
     sleep(1500)
     let xmsqAPP = className("android.widget.FrameLayout").packageName("com.xiaomi.vipaccount");
     let n = 0;
 ...
 function posts(n) {
     ...
     if (pkly) {
         ...
         while (!gz.findOne(2666)) {
             ...
             backAppIndex();
             // 坐标点击第一个帖子
             click(pkly.centerX(), pkly.centerY() * 0.9);
             // exit();
             // if (!gz.findOne(2666)) {
             //     toastLog(4)
             //     pkly.click();
 ...
 function backAppIndex() {
     ...
     while (backImg.exists()) {
         let dian = backImg.findOne().click();
         //click(dian.centerX(), dian.centerY());
         sleep(300)
     }
     //发现退回按钮
     ...
     while (xmsqAPP.exists() &&
         ...
         if (indexAble) {
             click(indexAble.centerX(), indexAble.centerY().centerY());
             indexAble.click();
 ...
 function see() {
     ...
     swipe(dwidth * 1 / 2, dheight * 4 / 5, dwidth * 1 / 2, dheight * 1 / 2, 500);
 ...
 function start() {
     var done = text("已签到").findOne(1500);
     try {
         if (!done) findCenter();
     } catch (e) {
 ...
 function test() {
     ...
     //let done = className("android.widget.TextView").text('使用小米社区App签到可获得额外成长值');
     ...
     notice(String(newdate), String(todayVal))
     exit();
 }
 ...
 function main() {
     //小程序签到();
     //level2();
     // notice(String('全部操作已完成(' + date.substr(5, 14) + ')'), String(level2()));
     //test();
     //posts(1);
     // exit();
     //return;
