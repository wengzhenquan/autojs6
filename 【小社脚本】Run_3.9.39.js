...
function posts() {
    ...
    // 检查是否是视频帖子
    k = pkly.siblingCount() - 1;
    let pre = pkly.previousSibling();
    while (k--) {
        if (!pre.className().includes('TextView') ||
            !pre.text().includes(':')) {
            pre = pkly.previousSibling();
            continue;
        }
...
function saveErroFile(path, salt) {
    ...
    //保留10个错误文件
    for (i = arr.length - 1; arr.length > config.保存错误截图数量; i--) {
