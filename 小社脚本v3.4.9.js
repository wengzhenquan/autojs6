...
function unLock() {
    ...
    //多次上滑
    for (i = 0; i < 3; i++) {
        swipe(dwidth * 5 / 8, dheight * 0.98, dwidth * 5 / 8, dheight * (0.6 - 0.2 * i), 300 * (i + 1));
        gesture(300 * (3 - i), [dwidth * 3 / 8, dheight * (0.98 - 0.2 * i)], [dwidth * 3 / 8, dheight * 0.3]);
        sleep(100);
    }
    log("上滑成功！");
    sleep(300);
...
function killAPP(packageN) {
    ...
    sleep(600)
    let i = 0;
    while (!existsOne(text("结束运行"), text("强行停止"), text("应用信息"), text("卸载"), text("清除数据"))) {
        if (text("应用详情").exists()) {
            back();
            sleep(500);
            break;
...
function findCenter() {
    ...
    // 等待验证图片的加载
    sleep(2000);
    let result = wait(() => existsOne(text("关闭验证"), text("刷新验证"), text("视觉障碍")), 5, 600);
...
function newSign() {
    ...
    while (urls[array[0]].includes('xcjd') ||
        (urls.length > 1 && urls[array[1]].includes('xcjd'))) {
...
function qd2(n) {
    ...
    if (notchWidth < cX(105))
        notchWidth = notchWidth + cX(random(3, 6));
...
function 小程序签到() {
   ...
   // log(1593)
    ...
    if (wait(() => me.exists(), 5, 600)) {
        ...
        //sleep(1500);
        if (wait(() => text('编辑资料').exists(), 6, 500)) {
            let qdbt = text("去签到");
...
function main() {
    ...
    events.on("exit", function() {
        ...
        if (config.结束震动) device.vibrate(config.结束震动);
    });
    try {
        //启动悬浮窗关闭按钮
        stopButton();
