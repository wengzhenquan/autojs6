...
function unLock() {
    ...
    log("开始解锁设备");
    wait(() => {
        swipe(dwidth * 1 / 2, dheight * 0.96, dwidth * 1 / 2, dheight * 3 / 5, 200)
        sleep(1000)
        return text("紧急呼叫").exists()
    }, 10, 200, {
        then() {
            log("上滑成功")
        },
            ...
            log("未找到解锁界面，签到退出")
            notice(String('出错了！(' + date.substr(5, 14) + ')'), String('未找到解锁界面'));
            exit();
        },
    });
    sleep(500);
    ...
    sleep(300);
    home();
    let jiesuo = contentMatch(/电话/);
    let result = wait(() => jiesuo.exists(), 10, 500)
    if (!result) {
        notice(String('出错了！(' + date.substr(5, 14) + ')'), String('屏幕解锁失败了！'));
        exit();
    }
}
...
function launchAPP(packageName) {
    ...
    // log(a)
    // let xmsqAPP = className("android.widget.FrameLayout").packageName(packageName);
    let n = 0;
    while (!waitForPackage(packageName, 500)) {
...
function see() {
    ...
    //swipe(dwidth * 1 / 2, dheight * 4 / 5, dwidth * 1 / 2, dheight * 1 / 2, 600);
    //gesture(1000, [dwidth * 1 / 2, dheight * 0.8], [dwidth * 1 / 2, dheight * 0.5]);
    //sleep(1000);
...
function level2() {
    ...
    if (num) {
        ...
        // 成长值获得错误
        // if (jilu.size() === 0 || jilu > 10) return level2();
        // 今日成长值
        let todayVal = 0;
...
function 小程序签到() {
    ...
    let xxcx = className("android.widget.ImageButton").desc("更多").packageNameMatches("com.tencent.mm");
    //小米社区5.3以上版本进入小程序
    ...
    if (v53) {
        ...
        while (!xxcx.exists()) {
            ...
            if (className("android.widget.TextView").text("通讯录").findOne(2666)) {
                ...
                //let n = 0;
                //while (!xxcx.exists()) {
                swipe(dwidth * 1 / 2, dheight * 1 / 5, dwidth * 1 / 2, dheight * 4 / 5, 300);
                let xlxcx = contentMatch(/小米社区/);
                // 去“更多”里寻找
                if (!xlxcx.exists()) {
        ...
        for (i = 0; i < 3 && !xxcx.exists(); i++) {
            if (config.坐标点击) {
                ...
                let xcx = desc("小米社区").findOne(1666);
                if ((null === xcx || !xcx) && i === 0) {
...
function start() {
    var done = text("已签到").findOne(2500);
    try {
        if (!done) findCenter();
    } catch (e) {
        notice(String('出错了！(' + date.substr(5, 14) + ')'), String("社区APP签到失败了！其它流程依旧继续进行……\n若反复该提示,可能账号出错了,快打开小米社区看看吧1"));
        /* 打印简单的错误消息. */
        /* 通常只有 1 行消息. */
...
function permissionv() {
    ...
    try {
        ...
        notice(String('截图权限无法通过，任务终止！'), String("请开启'投影媒体权限'！\n如已开启依旧如此，或许需要重启手机！"));
        console.error(e.message);
        exit();
        return;
    }
