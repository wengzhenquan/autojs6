...
function posts(n) {
    ...
    if (pkly) {
        ...
        let gz2 = className("android.view.View")
            .text("关注");
        while (!(gz.findOne(888) || gz2.findOne(888))) {
            if (n > 5) {
                toastLog("打开帖子失败！")
                unfinished_mark = 1;
                return;
            ...
            // 坐标点击第一个“评论”入口上方
            pkly = className("android.widget.ImageView")
                .desc("评论").findOne();
    ...
    if (wait(() => register.exists(), 4, 600)) {
        ...
        if (checkBox.click()) {
            ...
            if (qd) {
                ...
                //位置在右下角
                click(dwidth * 0.734, dheight * 0.884);
                click(dwidth * 0.734, dheight * 0.9);
            }
            sleep(1500)
        }
    }
    ...
    if (jpso.length > 0) {
        for (i = 0; i < jpso.length; i++) {
            ...
            let xyyx = className("android.widget.Button")
                .text("炫耀一下").findOne(1888);
            if (xyyx) {
                xyyx = xyyx.parent().nextSibling();
                while (!xyyx.clickable()) {
                    xyyx = xyyx.nextSibling();
                }
                xyyx.click();
                // 关闭图形❌组件
                clickCenter(xyyx);
                ...
                // 只能靠坐标关闭
                for (i = 0; i < 10; i++) {
                    click(dwidth * 0.5, dheight * (0.82 + 0.003 * i));
                    sleep(100);
                }
            }
            sleep(600);
...
function 小程序签到() {
    ...
    if (wait(() => me.exists(), 5, 600)) {
        ...
        //sleep(1500);
        if (wait(() => text('编辑资料').exists(), 5, 600)) {
            let qdbt = text("去签到");
...
function levelResult() {
    ...
    成长值记录.详细记录.forEach((record) => {
        const item = record.项目.length > 10 ?
            record.项目.padEnd(18) : record.项目.padEnd(28);
