...
function loginAccounts(accounts, password) {
    ...
    if (accounts && password) {
        ...
        sleep(500)
        log("输入账号：" + accounts)
        text('邮箱/手机号码/小米ID').findOne().setText(accounts);
        sleep(500)
    ...
    acc = id('mine_tv_nickname')
        .findOne(1000)
    if (!acc) {
        ableClick(className('Button').text('取消'))
        sleep(2000)
        acc = id('mine_tv_nickname')
            .findOne(1000)
