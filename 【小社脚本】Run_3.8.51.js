...
function skipAd() {
    ...
    while (n--) {
        //开屏广告
        if (wait(() => textStartsWith("跳过").exists(), 2) && n === 2) {
...
function 小程序签到(pram) {
    ...
    if (!wait(() => xxcx.exists(), 8, 600)) {
        if (wait(() => content('通讯录').exists() && content('发现').exists(),
                3, 500)) {
...
function getClipPic() {
    ...
    do {
        ...
        // 若找到参照组件，就更新参数
        let ycdj = textStartsWith("请在下图依次点击").findOne(3000);
        if (ycdj && tida) {
