...
function unLock() {
    ...
    log("开始解锁设备……");
    //两种方式重复上滑
    swipe(dwidth * 5 / 8, dheight * 0.98, dwidth * 5 / 8, dheight * 1 / 3, 1000);
    sleep(500);
    gesture(1000, [dwidth * 5 / 8, dheight * 0.98], [dwidth * 5 / 8, dheight * 1 / 3]);
    log("上滑成功！");
    sleep(300);
    home();
    sleep(300);
...
function launchAPP(packageN) {
    ...
    while (!packageName(packageN).findOne(500)) {
        ...
        if (n > 3) {
            // 从详情页启动小米社区
            let yyxx = text("应用信息").findOne(500);
            let yyxq = text("应用详情")
            if (yyxx) {
...
function posts(n) {
    ...
    if (pkly) {
        ...
        let gz2 = className("android.view.View")
            .text("关注");
        while (!(gz.findOne(1666) || gz2.findOne(1666))) {
            if (n > 5) {
                toastLog("打开帖子失败！")
                unfinished_mark = 1;
                return;
            ...
            toastLog("第" + n + "次重试")
            // 误点开图片
            let bc = className("android.widget.TextView")
                .text("保存");
            if (bc.exists()) {
                back();
                sleep(500);
            ...
            // 点开视频贴
            let vp = idStartsWith('videoPlayer_')
                .text("视频播放器");
            if (vp.exists()) {
                back();
            }
...
function findCenter() {
    ...
    // 等待验证图片的加载
    sleep(2000);
    let result = existsOne(text("关闭验证"), text("刷新验证"), text("帮助反馈"), text("视觉障碍"));
...
function webTest() {
    ...
    console.error("提示：签到识图服务器准备就绪");
    console.error("检查服务器耗时：" + time + " ms");
...
let urls = [
    ...
    "http://xmst.wzqw.zone.id/upload", //1
    "http://up.kuandana.v6.rocks/upload", //2
    "http://xcjd.wzq.dpdns.org/upload", //3

    "http://strz.wzq.dpdns.org/upload", //4
    "http://xmst.wengzq.v6.rocks/upload", //5
    "http://up.风中拾叶.top/upload", //6
    "http://xcjd.wzqw.zone.id/upload", //7

    "http://strz.wzqw.zone.id/upload", //8
    "http://xmst.wzq.dpdns.org/upload", //9
    "http://xcjd.wengzq.v6.rocks/upload", //10
...
function newSign() {
    ...
    let delayed_bak = delayed; //备份超时时间

    let clipParam = getClipParam();
    while (true) {
        //慢服务器，时间改成最大超时时间
        if (url.includes('xcjd'))
            delayed = delayed_max;
        else delayed = delayed_bak; // 恢复超时

        log("开始第" + (i + 1) + "次申请");
        try {
            let res = upload(url, clipParam);
...
//上传图片
function upload(url, clipParam) {

    //再次截图，以便传输
    var pictures2 = images.read("./tmp/pictures2.png");
    var pic = images.clip(pictures2,
        clipParam.x, clipParam.y,
        clipParam.w, clipParam.h);
...
function see() {
    ...
    sleep(1000);
    let blb = className("android.widget.Button").text("查看金币详情")
    if (wait(() => blb.exists(), 3, 600)) {
        toastLog("拔萝卜签到成功", "forcible");
    }
    back();
}
...
function 活动1() {
    ...
    let register = className("android.widget.Button")
        .text("立即报名").findOne(1500);
    if (register) {
        sleep(500)
        // 勾选 “我已阅读并同意”
        let checkBox = register.nextSibling();
        if (!checkBox.clickable()) checkBox = checkBox.firstChild();
...
function 小程序签到() {
    ...
    if (!wait(() => xxcx.exists(), 3, 600) &&
        ...
        let fh = className("android.widget.ImageView")
            .desc("返回").findOne(666);
        if (fh) fh.click();
...
function level2() {
    ...
    if (!num) {
        toastLog("布局分析失效，未找到成长值数据！", "forcible");
...
function test() {
    ...
    if (!xxcx.exists() &&
        ...
        let xlxcx = descStartsWith('ai码字');
        // 去“更多”，最近使用小程序，里寻找
        if (!wait(() => xlxcx.exists(), 5, 500)) {
            className('android.widget.RelativeLayout')
                .desc('更多小程序').clickable().findOne().click();
            let m = 0;
            while (!xlxcx.exists()) {
                swipe(dwidth * 0.6, dheight * 0.8, dwidth * 0.4, dheight * 0.2, 500);
                m++;
