...
function findCenter() {
    ...
    if (!result) {
        console.error("芭比Q了，点不开签到页面！", "forcible");
        throw e;
        return;
    }
    //屏幕截图
...
//------------ 活动 ----------//
//拔萝卜活动
function pullingCarrots() {
    while (!packageName(xmPckageName).exists()) sleep(500);
    console.info(">>>>>>>→萝卜活动←<<<<<<<")
    toastLog("拔萝卜活动签到……", "forcible");

    var button = className("android.widget.Button").content("去看看");
...
function ganenji() {
    ...
    let qucanyu = className("android.widget.Button").content("去参加");

    // 没找到入口
    if (!wait(() => qucanyu.exists(), 5, 500)) {
        toastLog("未找到活动入口，重新进入", "forcible")
        back();
        sleep(1000);
        // 小米社区重置首页
    ...
    if (!ableClick(qucanyu.findOne(1500))) {
        log('尝试寻找其它入口')
        back();
        //返回首页外层
        backAppIndex();
...
function dualFlagships() {
    while (!packageName(xmPckageName).exists()) sleep(500);
    console.info(">>>>>>>→旗舰活动←<<<<<<<")
    toastLog("开始双旗舰活动……", "forcible")
    let cj = className("android.widget.Button").content("去参加");
...
function level1() {
    ...
    //while (scrollUp());
    swipe(dwidth * 0.5, dheight * 0.4, dwidth * 0.5, dheight * 0.8, 300); // 向上滚动查找
    sleep(1000);
    log('进入成长值页面')
    if (!ableClick(button.findOne(2000))) {
        console.error('未找到入口')
...
function levelResult() {
    ...
    let outP1 = (`成长值：${成长值记录.当前成长值}(${成长值记录.当前等级()})`).padEnd(15) + (`升级目标：${成长值记录.升级目标}`);
...
function run() {
    ...
    if (launchAPP(xmPckageName)) {
        ...
        if (wait(() => sign.exists(), 5, 888)) {
            ...
            if (config.拔萝卜) pullingCarrots();
            //  if (config.米粉节) fans();
            //  if (config.观看视频) watchVideo();
            if (config.双旗舰) dualFlagships();
