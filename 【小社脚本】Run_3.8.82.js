...
function 解锁() {
    ...
    if (wait(() => register.exists(), 4, 600)) {
        ...
        if (ableClick(checkBox)) {
            ...
            if (!ableClick(qd)) {
                ...
                click(dwidth * 0.734, navBarY - cY(100));
            }
            sleep(1500);
        }
    }
    ...
    if (!ddjs && !jpso) {
        ...
        console.warn('滑到最下面↓')
        for (let i = 0; i < 5; i++) {
            swipe(dwidth * (2 + i) / 8, dheight * (0.5 + 0.02 * i), navBarY * (2 + i) / 8, navBarY * (0.3 + 0.02 * i), 100);
...
function 小程序签到(pram) {
    ...
    while (k-- && !wait(() => packageName(wchatpn).exists(), 10, 600)) {
        ...
        if (isEnabled && v53 && !config.坐标点击) {
            ...
            openVChat(v53);
            if (!wait(() => packageName(wchatpn).exists(), 5, 600))
                launchAPP(wchatpn);
        } else {
            desktopRun();
        }
    ...
    if (wait(() => textContains('版本较低').exists() || !me.exists(), 4, 600)) {
        if (wait(() => textContains('版本较低').exists(), 2, 600)) {
            ...
            for (let i = 0; i < 3; i++) {
                click(dwidth * 0.5, navBarY * (0.53 + 0.03 * i));
            }
            sleep(1500);
            click(cX(50), statusBarHeight + cY(60));
            sleep(1000);
        }
        sleep(2000);
...
function saveXcXButton(obj) {
    ...
    try {
        ...
        if (obj && (obj instanceof UiObject)) {
            ...
            auto_button.set("button_name", obj.text())
...
function levelResult() {
    ...
    let 还需时日 = Math.ceil(距离升级还需 / (今日获得 < 9 ? 今日获得 : 5));
