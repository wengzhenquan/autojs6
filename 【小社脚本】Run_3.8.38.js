...
function ganenji(pram) {
    ...
    if (!signEntrance('周年感恩', pram)) {
        ...
        sleep(1000)
        log('进入活动页面')
        if (!ableClick(descContains('周年感恩'))) {
...
function 小程序签到(pram) {
    ...
    if (!wait(() => xxcx.exists(), 8, 600)) {
        if (wait(() => text('通讯录').exists() && text('发现').exists(),
                3, 500)) {
...
function findCenter(pram) {
    ...
    //captureScr();
    if (config.社区APP签到方式 === 1 ||
        textStartsWith("请在下图依次点击").findOne(1500)
    ) {
        // 未初始化
...
function getClipPic() {
    ...
    do {
        // 若找到参照组件，就更新参数
        var ycdj = textStartsWith("请在下图依次点击").findOne(1500);
        if (ycdj) {
            const bottom = ycdj.bottom();
            //找到合适的父组件
            while (
                ycdj.right() < dwidth * 0.6 ||
                (ycdj.parent() &&
                    // ycdj.parent().right() < dwidth &&
                    ycdj.parent().bottom() < dheight * 0.45 &&
        ...
        var tida = className("android.widget.Button")
