...
function findCenter() {
    ...
    let ycdj = className("android.widget.TextView").textStartsWith("请在下图依次点击").findOne(1500);

    if (ycdj) {
        unfinished_mark = 1;
        //识图点击认证
        let ckbj = ycdj.bounds();
        //未完成逻辑
        console.error("社区APP的识图签到还没实现！");


        notice(String('社区签到没完成！(' + nowDate().substr(5, 14) + ')'), String("芭比Q了，程序员还没实现这种签到！"));
    } else {
        //滑块认证
        auth_hk();
    }
