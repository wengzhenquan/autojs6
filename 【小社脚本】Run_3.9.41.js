...
//  开盒页面X按钮
const x_bt_center = "./tmp/x_bt_center.json";
...
// 记录成长值
const gain = storages.create('gain');
...
const 成长值记录 = {
    ...
    预估日得() {
        ...
        if (total > 0) {
            ...
            } else if (update_date !== today || total > todayUsedValue) {
...
function killAPP(packageN) {
    ...
    if (qx && !qx.isSingleton()) {
        ...
        while (qd.className() !== qx.className() || qd.text() === '') {
            let qdn = qd.nextSibling();

            if (qdn.id() !== qx.id() && qdn.text() !== qx.text())
                qd = qdn;
            else break;
        }
        log("点击：" + qd.text())
...
function launchAPP(packageN) {
    ...
    let n = 0;
    // 这个循环走一遍至少需要1秒
    while (!checkCurrentApp(packageN, 0)) {
        //链式调用权限触发，点“始终允许”
        ...
        if (checkCurrentApp(packageN, 0)) break;
        ...
        if (dualRow === 0) {
            ...
            // 分身
            runAppClone(packageN)
        }



        if (n > 5) {
            if (n < 7 && text('打开').exists() &&
                text(appName).exists() &&
            ...
            if (checkCurrentApp(packageN, 6000)) break;
...
//跳过广告
function skipAd() {
    while (!checkCurrentApp(xmPckageName, 0)) sleep(500);
    ...
    while (n--) {
        ...
        if (readed.exists()) {
            ...
            if (textContains('验证').exists() || readed.exists()) {
...
function rePosts() {
    let button = className("android.widget.Button")
        .content('去浏览').findOne(1500);

    if (!button)
        return;

    console.error('发现浏览帖子失败！')
    console.error('再次浏览帖子……')
...
function posts() {
    ...
    //backAppIndex();
    let k = 0;
    let pkly = descEndsWith("评论").find(3000);
    while (!pkly) {
        ...
        swipe(dwidth * 0.5, dheight * 0.4, dwidth * 0.5, dheight * 0.8, 200);
        wait(() => false, 2000);
        pkly = descEndsWith("评论").find(2000);
        k++;

    }

    let targetIndex = -1;
    let maxRetry = 5; // 最大重试次数，防止无限循环
    let retryCount = 0;

    while (targetIndex === -1 && retryCount < maxRetry) {
        // ========== 查找逻辑（扁平化，无嵌套函数）==========
        for (let i = 0; i < pkly.length; i++) {
            let w = pkly[i];
            let parent = w.parent();
            if (!parent) continue;

            let siblings = parent.children();
            let hasTimeSibling = false;

            for (let j = 0; j < siblings.length; j++) {
                if (siblings[j] === w) continue;
                let txt = siblings[j].text();
                if (txt && txt.length === 5 && /^\d{2}:\d{2}$/.test(txt)) {
                    hasTimeSibling = true;
                    break;
                }
            }
    ...
    // ========== 结果处理 ==========
    if (targetIndex !== -1) {
        log("✅ 找到目标，索引: " + targetIndex + "，共重试 " + retryCount + " 次");
        // let targetNode = pkly[targetIndex];
        // targetNode.click();
    } else {
        log("❌ 重试 " + maxRetry + " 次后仍未找到目标");
        targetIndex = 0;
    }


    ableClick(pkly[targetIndex].parent());
    sleep(1500);
...
function xcxRegEvent() {
    ...
    // 社区APP
    if (!hasRegEvent &&
        wait(() => checkCurrentApp(xmPckageName, 7000), 2)) {
...
function toSign() {
    ...
    if (!checkCurrentApp(xmPckageName, 7000)) {
        toastError("未打开社区APP", "forcible")
        return false;
    }
    if (text('签到').exists()) return;
...
function signEntrance(name, pram) {
    let nb = contentContains(name);
    if (!wait(() => nb.exists(), 3, 1000)) {
        clearCache();
    ...
    if (!nb.clickable()) {
        ...
        let n = 3;
        while (n-- && (nb.text() !== pram && nb.desc() !== pram)) {
            nb = nb.nextSibling()
        }
        if (nb.text() !== pram && nb.desc() !== pram) {
            console.error('没找到按钮：' + pram)
            return false;
        }
        log("点击按钮→" + pram)
    }
    ...
    if (pram !== '立即签到' && !pram.includes('加载中')) {

        ableClick(contentStartsWith(pram));
...
//开盒活动
function kaihe(pram) {
    if (pram === 0 || pram === '0') return;
    if (pram === 1 || pram === '1') {
        pram = '去参加';
        if (content('去参与').findOne(1000))
            pram = '去参与';
    }

    while (!checkCurrentApp(xmPckageName, 0)) sleep(500);
    console.info(">>>>>>→开盒活动←<<<<<<")
    toastLog("通用开盒活动签到……", "forcible");
...
function 自定义开盒(name, pram) {
    ...
    while (!checkCurrentApp(xmPckageName, 0)) sleep(500);
    console.info(">>>>>→[" + name + "]活动←<<<<<")
    toastLog("开始[" + name + "]活动……", "forcible")
    if (!signEntrance(name, pram)) {
        //返回首页外层
        backAppIndex();
        log('→我的')
        ableClick(text("我的"));
        ...
        sleep(1000)
        log('进入活动页面')
        if (!ableClick(descContains(name).findOne(5000))) {
            console.error('找不到活动入口')
            return;
        }
    }
...
function 开盒() {
    ...
    if (!ddjs && !jpso) {
        ...
        do {
            ...
            // 关闭炫耀一下
            if (result) {
                let x_button = result.parent().nextSibling();
                saveXButton(x_button);
                ableClick(x_button);
                // 关闭图形❌组件
                //clickCenter(xyyx);
            } else {
...
// 读取X按钮中心坐标
function getXButton() {
    if (!files.exists(x_bt_center))
        return false;

    try {

        let button = JSON.parse(files.read(x_bt_center));
...
function 小程序签到(pram) {
    ...
    while (k-- && !checkCurrentApp(wchatpn, 7000)) {
        ...
        if (isEnabled && v53 && !config.坐标点击) {
            ...
            // 如果没打开小程序，就手动启动微信
            if (!checkCurrentApp(wchatpn, 7000)) {
                launchAPP(wchatpn);
                sleep(5000)
            }
        } else {
    ...
    if (!activity.includes('plugin.appbrand.ui.AppBrand') &&
        activity.includes(wchatpn) &&
        (content("通讯录").exists() || desc("返回").exists() ||
            descStartsWith("更多功能").exists())
    ) {
    ...
    let bjfx = false; // 布局分析
    let wzdl = text('我知道了').exists();
    if (wzdl || xxcx.exists()) {
        wzdl && ableClick(text('我知道了')) &&
            sleep(500)
        bjfx = true;
    } else {
        // 布局分析失效，点“我知道了”
        if (config.我知道了坐标) {
            log("[我知道了]坐标：" + config.我知道了坐标)
            log("配置[isee_x：" + config.isee_x + "]")
            log("配置[isee_y：" + config.isee_y + "]")
            click(config.isee_x, config.isee_y);
        } else
            click(dwidth * 0.5, navBarY * 0.53);

        sleep(2000);
        if (config.左上角返回) {
            log("左上角返回图标：" + config.左上角返回)
            log("配置[returned_x：" + config.returned_x + "]")
            log("配置[returned_y：" + config.returned_y + "]")
            click(config.returned_x, config.returned_y);
            sleep(500)
            click(config.returned_x, config.returned_y);
        } else {
    ...
    if (wait(() => me.exists(), 5, 500)) {
        ...
        if (!config.右下角我的坐标) {
            ...
            log("右下角[我的]坐标：" + config.右下角我的坐标)
            log("配置[my_x：" + config.my_x + "]")
            log("配置[my_y：" + config.my_y + "]")
            click(config.my_x, config.my_y);
            sleep(500);
            click(config.my_x, config.my_y);
        }
...
function openVChat(button) {
    ...
    while (n--) {
        ...
        if (wait(() => textContains("选择").exists(), 3, 600)) {
            // 存在微信分身，选择第1个
        ...
        if (xcx.length > 0) {
            ...
            if (xcx.length > 1) {
                ...
                for (i = 0; i < xcx.length; i++) {
                    ableClick(xcx.get(i));
                    log("正在尝试打开→第[ " + (i + 1) + " ]个！");
                    if (wait(() => checkCurrentApp(xmPckageName, 7000), 6, 500)) {
...
function start(pram) {
    ...
    var done = content("已签到");

    while (!done.exists() && !content(pram).exists()) sleep(1000)
...
function findCenter(pram) {
    ...
    if (!contentStartsWith(pram).findOne(1666) &&
        contentStartsWith('加载中').findOne(1666))
        pram = '加载中';
...
function saveErroFile(path, salt) {
    ...
    //保留10个错误文件
    for (i = arr.length; arr.length > config.保存错误截图数量; i--) {
...
function loginAccounts(accounts, password) {
    ...
    if (wait(() => textContains('验证').exists(), 2, 600)) {
        console.error("登录失败！")
        console.error("需要验证码")
        if (config && config.通知提醒)
    ...
    while (!acc) {
        ...
        if (text('小米账号登录').exists() || textStartsWith('已阅读').exists()) {
...
function run() {
    ...
    if (!app.getAppName(xmPckageName)) {
    ...
    for (let row = 0; row <= runLen; row++) {
        ...
        if (launchAPP(xmPckageName)) {
            ...
            if (toSign()) {
                ...
                if (!config.执行顺序) {
                    if (config.通用开盒活动) kaihe(config.通用开盒活动);
                    if (config.自定义开盒活动 &&
                        config.自定义开盒活动.length > 0 &&
                        Array.isArray(config.自定义开盒活动)) {
                        config.自定义开盒活动.forEach(item => {
                ...
                if (config.执行顺序) {
                    if (!checkCurrentApp(xmPckageName, 0)) {
                        //回到小米社区app
                        launchAPP(xmPckageName);
                        toSign();
                    }
