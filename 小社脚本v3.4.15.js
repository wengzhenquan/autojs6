...
function unLock() {
    ...
    //多次上滑
    for (i = 0; i < 2; i++) {
        swipe(dwidth * 5 / 8, dheight * 0.99, dwidth * 5 / 8, dheight * (0.6 - 0.2 * i), 300 * (i + 1));
        gesture(300 * (3 - i), [dwidth * 3 / 8, dheight * (0.99 - 0.3 * i)], [dwidth * 3 / 8, dheight * 0.3]);
    }
    log("上滑成功！");
    sleep(300);
    home();
...
function 解锁(button) {
   ...
   // if (1) {
        ...
        log("解锁最后一个");
        for (i = 0; i < 3; i++) {
            click(dwidth * 0.88, dheight * (0.63 - 0.1 * i));
            sleep(300);
        }
        // 关闭提示
        for (i = 0; i < 5; i++) {
        ...
        log("再解锁一个");
        for (i = 0; i < 3; i++) {
            click(dwidth * 0.13, dheight * (0.63 - 0.1 * i));
            sleep(300);
        }
