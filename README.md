...
* $\color{red}{如果担心使用脚本会导致黑号，请不要使用此脚本。}$  
...
***主程序启动时，若需要切换历史版本，只需修改version文件的run值即可：***  
``` 
   
"run": "【小社脚本】Run_3.7.x.js",
...
### **5. 编辑配置文件`config.js`：**  
   ...
   * 配置文件里面都有详细说明：  
     ...
     * **数字(或混合)密码：** 将“解锁方式”改成2
       * $\color{red}{数字密码}$ ：“000000”改成你的数字密码
       * $\color{red}{混合密码(测试)}$ ：“000000”改成你的混合密码，仅支持 $\color{red}{数字和小写字母}$ 组合。
...
### 1. $\color{red}{布局分析失效}$ 是什么意思？   
   ...
   * 尝试开启：config配置→ $\color{red}{fast模式: 改成1}$  
...
### 2. 为什么明明已经打开小程序，依旧报“ $\color{red}{进不了微信小程序，我尽力了！}$ ”？  
...
### 3. 什么是“ $\color{red}{排水沟过弯大法}$ ”？我的手机有时候无法启动小米社区？
...
### 7.为什么我点击`【停止脚本】`按钮无法停止脚本？显示“ $\color{red}{关不掉！x1}$ ”。  
...
### 11. $\color{red}{方法plugins.load调用失败}$。  
...
### 13. $\color{red}{打开应用详情页失败了！}$  
   
   无障碍权限掉了，关闭无障碍权限，重新授权开启一下。
   
### 14. 启动不了。日志只有“开始运行”、“结束运行”。  
