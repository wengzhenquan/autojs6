...
* $\color{red}{[小社脚本]一键更新程序.js 启动即可自动更新脚本}$   
1. 依赖version文件
2. 自带github加速下载
3. 若需要更新config.js，会自动备份为config_old.js
3. 不能确保更新的文件不会损坏

旧版脚本使用一键更新程序.js方法：
``` 
1、下载version、[小社脚本]一键更新程序.js
2、将version文件里面所有值改成1，
  updateFile对应本地已存在的文件清单和路径，
  没有的删掉，有的修改添加。
3、启动 [小社脚本]一键更新程序.js
这将会全量更新
```  
...
**7.完成！试跑！定时任务！**   
  ...
  * 试跑：  
   ...
   ```  
    需保持 AutoJs6 后台运行, 
    包括 [ 自启动白名单 / 忽略电池优化 / 忽略后台活动限制 / 系统多任务保留 ] 等.   
   ```  

  * **$\color{red}{执行过程可能发生错误。}$**  
    出现任何问题，先查看本页面下方的 **问题收集** ，下面收录了大多数遇到的问题，并且同时也在更新。   
    
  * 保证全自动化进行，必须打开 **$\color{red}{“投影媒体权限”}$**！   
...
## 问题收集：
   ...
   **5.无障碍服务已启用但未运行/无障碍服务老掉**  
   ...
   ```
   AutoJS6→设置：
   使用修改安全设置权限自动启用无障碍服务→启用修改安全设置权限
   使用root权限自动启用无障碍服务
   ```
