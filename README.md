...
*   $\color{red}{小提示：APP脚本打包版，地址在下面第10步！}$ 

******************************************************************

原作者@PJxiaoyu、@fgvsuiye

社区APP签到，识别图片坐标程序放到了服务器上，没办法，AutoJS6不支持调用pt模型。  
只是上传一张截图（tmp/pic.png），绝对没有窃取用户信息，代码都是开源的，欢迎检阅。  
有没有办法把pt模型让AutoJS6直接调用，弄到本地来？  
如果有对这方面比较精通的，可以联系我。  
模型是YOLO的。  
...
**6. 小米社区小程序图标，必须放在桌面**  
   **$\color{red}{（小米社区APP若有入口，可忽略该操作）。}$**  
   ...
   $\color{red}{坐标点击: 0,}$ ：从桌面自动搜索小图标，无论放在哪一屏（**推荐**）。  

   $\color{red}{坐标点击: 1,}$ ：从第3屏指定坐标点击启动小程序，坐标获取方法与上面一样（**不推荐**）。**（如果你确定这样做了，那么图标必须放在第3屏！）**     
...
试跑：  
 ...
 调试成功后，建议设置 $\color{red}{2个定时任务}$，原因如下：   
  1. 小程序签到在最后面，说不定小程序签到完，成长值增长，又足够开盒活动！   
  2. 如果遇到小程序布局分析失效，会导致小程序签到获得的成长值没有统计，需要从社区APP重新获取。  
