...
### **2. 更改autojs6工作路径。**  
   ...
   **$\color{red}{如果会卡音量键，就把音量键相关的设置全关闭}$**  
   **$\color{red}{下面还有个“音量-”，也关闭}$**  
...
### **5. 编辑配置文件 $\color{red}{config.js}$：**  
   ...
   * 解锁流程说明：  
     * $\color{red}{屏幕亮屏至少2次}$  
       1. 第1次亮屏，激活设备   
       2. 然后检查权限、检查更新（5~10秒）   
         （如果需要更新脚本，会在锁屏状态下后台更新，然后重新运行新程序，这可能需要1分钟）   
       3. 第2次亮屏，上滑解锁  
       4. 如果上滑失败，会再次重复[息屏→亮屏→上滑]   
     * $\color{red}{如果超过3分钟未解锁，却反复息屏、亮屏，极有可能解锁失败}$
       1. 查看日志，可尝试将配置[上滑起始位置]改成1
       2. 反馈问题
...
### **6. 安装本地YOLO签到模块**  
   ...
   * 本地YOLO识图: 1，**(推荐，识别成功率95\%以上):** 
     * 需 $\color{red}{安装Yolo-plugin.apk}$插件，文件在yolov11目录下。($\color{red}{若没看到文件，可下滑刷新}$) 
     ...
     ![image](说明/插件中心_筛选.jpg)   
     ![image](说明/插件中心_筛选2.jpg)   
     $\color{red}{Autojs6低于6.7.0版本不需要插件中心设置}$  
...
### **~7. 小米社区小程序图标，必须放在桌面~**  
   ...
   * 图标坐标获取（布局分析工具）：
     ...
     2. $\color{red}{center[x，y]:矩形中心坐标，选这个坐标}$
...
### **9. 扩展功能：批量签到、小号**   
   ...
   * 分身：  
     ...
     **$\color{red}{社区APP分身如果不支持布局分析，也不支持。}$**     
     **$\color{red}{目前所知，支持操作系统自带的分身功能（比如华为、一加）}$**  
...
#### 1. $\color{red}{布局分析失效}$ 是什么意思？   
   ...
   * 尝试关闭无障碍权限→重新授权无障碍权限→重启手机  
   ...
   ```  
   * $\color{red}{如果无法解决布局分析失效，别慌，也能用！！！}$    
     脚本会按照坐标点击 $\color{red}{(摸黑操作！！！)}$。   
     脚本已经内置点击坐标，如果点击错误，`config`里面有几项可以改坐标。  
     按照要求，先把开关改成1(启用），然后修改下面的坐标，即可生效。   
     ![image](说明/更改坐标.jpg)  
     $\color{red}{不用全改，哪个操作需要改，就改哪里。}$ 
     $\color{red}{能用就别改！}$ 
     
     前面介绍了两种坐标获取方式：   
     * 5.编辑配置文件→图形锁的坐标获取方式
     * 7.小米社区小程序图标，必须放在桌面→图标坐标获取（布局分析工具）
...
#### 11. **方法plugins.load调用失败**。  
   ...
   如果提示 $\color{red}{未在插件中心授权/启用。}$   
   Autojs6 6.7.0版本开始有了插件管理，在软件上找找。   
   如果找不到，前面有图片告诉你插件中心的位置，往前翻→6. 安装本地YOLO签到模块
...
#### 13. 识图签到一直失败/点击顺序错误/识别的图标数量3/2/1/0。   
   ...
   * 如果pictures2.png、pic.png截图都是黑屏
     * 因为系统开启了屏幕保护，不允许在屏幕共享的模式下截图，    
       只需将社区APP加入截图白名单，或者关闭屏幕保护即可
...
#### 17. **上滑失败，重试！**  
   ...
   如果通过几次重试能解锁成功，可以尝试增大 **$\color{red}{上滑时长基数}$**  
   config→上滑时长基数:（默认115）   
   建议值：115~255  
   可以尝试改成155、165、205、255 等，再大就不建议了   
...
#### 20. **定时任务不启动，连日志也空白**  
   **$\color{red}{尝试开启“前台服务”或重启手机}$**  
   如果是6.7.0以上版本：Autojs6设置→定时任务调度引擎，尝试更换引擎
   
#### 21. **社区APP版本过低！同时微信和社区版本提示[未安装]**  
   **$\color{red}{尝试更换Autojs6版本}$**  
