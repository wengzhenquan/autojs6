...
function consoleShow() {
    if (config.悬浮窗控制台) {
        ...
        //console.setContentTextColor({log: 'white'});
        console.show();
    }
}
...
function unLock() {
    ...
    log("开始解锁设备……");
    //两种方式重复上滑
    swipe(dwidth * 5 / 8, dheight * 0.98, dwidth * 5 / 8, dheight * 1 / 4, 1000);
    gesture(1000, [dwidth * 5 / 8, dheight * 0.98], [dwidth * 5 / 8, dheight * 1 / 4]);
    log("上滑成功！");
    sleep(300);
    home();
    sleep(300);
...
function posts(n) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        gesture(1500, [dwidth * 1 / 2, dheight * 0.8 + ran], [dwidth * 1 / 2, dheight * 0.8 - ran]);
...
function qd2(n) {
    ...
    // 保存处理后的图片到脚本的同级目录
    images.save(img3, "./tmp/screenshot.png");
    ...
    var point3 = findColor(binaryGapImage, "#000000", {
        region: [point2.x + 96, 0, (binaryGapImage.width - (point2.x + 96)), 60],
        threshold: 4
    });
    if (point3 === null || point2.x === point3.x) {
        binaryGapImage.recycle();
        n++;
        return qd2(n); // 截图高度上移
    }
    binaryGapImage.recycle();
    let longx = point3.x - point.x + ran;
    log("需要移动的距离为：" + longx);
    log("终点坐标：(" + (longx + sliderCenter[0]) + "," + sliderCenter[1] + ")")

    //签到！滑动！
    toastLog("开始模拟滑动……", "forcible");
    gesture(2000, sliderCenter, [(longx + sliderCenter[0]), sliderCenter[1]]);
