...
function unLock() {
    ...
    let result = wait(() => existsOne(desc('第1屏'), desc('电话'), desc('短信'), '微信', '小米社区'), 10, 500);
    if (!result) {
...
function 小程序签到() {
    ...
    if (v53) {
        ...
        while (!packageName(wchatpn).exists()) {
            ...
            m++;
            sleep(500);
            if (packageName(wchatpn).findOne(666) || 3 > 10) break;
    ...
    if (wait(() => packageName(wchatpn).exists(), 10, 500) &&
        !xxcx.exists() &&
        (text("通讯录").findOne(2666) || desc('更多功能按钮').findOne(2666))) {
    ...
    if (!wait(() => xxcx.exists(), 5, 500)) {
        //无法通过打开小程序验证，可能因为小程序布局分析失效导致
        //但通过了前面打开微信的验证，
        // 不能确保成功打开小程序
        if (config.跳过小程序打开验证) {
            //再次尝试从桌面打开一次，以确保能顺利打开小程序
            //如果不想做这一步，可以注释掉下面4行代码
            toastLog("无法验证小程序是否已打开，只好再次打开一次！", "forcible")
            home();
            sleep(666);
            desktopRun();
    ...
    if (!me.findOne(3666)) {
        ...
        click(dwidth * 0.817, dheight * 0.49);
        sleep(1000);
        toastLog("【不确定操作】摸黑→签到不知道有没有完成！")
...
//小程序版成长值
function level2() {
    toastLog("【小程序】：我正在记录成长值……", "forcible")
    //let me = className("android.widget.TextView").text("我的");
    // 发现在子页面
    //if (wait(() => packageName(wchatpn).exists(), 10, 500) &&
    //    !text("我的").exists()) back();
    ...
    if (!num) {
        toastLog("布局分析失效，小程序未能找到成长值！(ㄒoㄒ) ", "forcible");
        toastLog("【不确定操作】摸黑→点击[成长值]", "forcible");
        click(dwidth * 0.5, dheight * 0.5);
    ...
    sleep(1000);
    if (!wait(() => text("每日签到").exists(), 5, 500)) {
        //组件无法识别，无法继续了！除非返回重新进来
        log("成长值明细页面，布局分析失效！无法获得明细。");
        back();
        return;
    }
...
function levelResult() {
    ...
    成长值记录.详细记录.forEach((record) => {
        const item = record.项目.padEnd(35);
...
function main() {
    ...
    sleep(300);
    home();
    notice(String('全部操作已完成(' + nowDate().substr(5, 14) + ')[' + getDurTime(date) + ']'), String(levelResult()));
