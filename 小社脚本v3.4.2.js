...
log(("微信 Version：").padEnd(25) + wchatVersionName)
log(("小米社区 Version：").padEnd(21) + xmVersionName)
...
function stopButton() {
    ...
    window.setPosition(dwidth * 0.1, dheight * 0.8)
    //悬浮窗被关闭时停止脚本
    window.exitOnClose();
    window.action.click(() => {
        window.close();
...
function launchAPP(packageN) {
    ...
    while (!packageName(packageN).findOne(500)) {
        ...
        //链式调用权限触发，点“始终允许”
        let dk = className("android.widget.TextView")
            .textContains("想要打开");
        if (dk.exists()) {
            sleep(200)
        ...
        if (n > 3) {
            ...
            let yyxx = className("android.widget.TextView")
                .text("应用信息").findOne(500);
            let yyxq = className("android.widget.TextView")
                .text("应用详情")
            if (yyxx) {
            ...
            if (yyxq.exists()) {
                let run1 = className("android.widget.TextView")
                    .textContains("启动");
                while (!run1.exists()) {
                    // 上滑寻找“启动”
...
function findCenter() {
    ...
    //点击签到按钮
    let qdbt = className("android.widget.TextView")
        .text("立即签到");
    // 没找到入口
    ...
    let ycdj = className("android.widget.TextView")
        .textStartsWith("请在下图依次点击").findOne(2500);
...
function newSign() {
    log("将会请求服务器上的程序来识别坐标！")
    // 发送请求，获取坐标
    let urls = [
        "http://strz.wengzq.v6.rocks/upload",
        "http://up.kuandana.v6.rocks/upload",
        "http://strz.wzqw.zone.id/upload",
        "http://up.风中拾叶.top/upload",
        "http://strz.wzq.dpdns.org/upload"

        // "http://kuandana.v6.rocks/upload"
    ];
    let i = 0,
        n = 0,
        u = 0;
    let url = urls[u];
    //let url = "http://35.220.200.166:5000/upload";
...
function logpercentage() {
    var percentageUi = className("android.widget.TextView")
        .textContains("当前签到+1的概率：").findOne(1500)
    if (percentageUi) {
...
function 解锁(button) {
    ...
    let jpso = className('android.widget.TextView')
        .text('可解锁').find();
...
function 小程序签到() {
    ...
    if (!wait(() => xxcx.exists(), 2, 600) &&
        ...
        // 微信下滑
        let zjsy = className("android.widget.TextView")
            .text("最近使用小程序");
    ...
    if (wait(() => me.exists(), 4, 500)) {
        ...
        sleep(1500);
        let edit = className("android.widget.TextView")
            .text('编辑资料');
        if (edit.exists()) {
            let qdbt = className("android.widget.TextView")
                .text("去签到");
...
function level2() {
    ...
    let jilu = className("android.widget.TextView")
        .text(newdate).find();
    if (jilu.length < 1) {
...
function level1() {
    ...
    let button = className("android.widget.TextView")
        .text("社区成长等级");
    // 没找到入口
    ...
    button.click();
    sleep(1000);
    let czz = className("android.widget.TextView")
        .textStartsWith("成长值");
