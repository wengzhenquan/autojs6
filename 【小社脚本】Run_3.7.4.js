...
function level1() {
    while (!packageName(xmPckageName).exists()) sleep(500);
    // while (!existsOne('签到', text('今天'), text('全新升级')));
    //  while (scrollUp());
    ...
    ableClick(button);
    clickCenter(button);
    while (!existsOne(text('我的成长值'), text('1段'), textStartsWith('成长值')));
    sleep(1000);
...
function run() {
    ...
    if (launchAPP(xmPckageName)) {
        ...
        if (wait(() => sign.exists(), 5, 888)) {
            ...
            sleep(666);
            while (click(text('刷新')) || click(text('重新加载')));
